...
public class CrystalUtil {
  ...
  public static float calculateDamage(double posX, double posY, double posZ, Entity entity, Vec3d vec) {
    ...
    double distanceSize = getRange(vec, posX, posY, posZ) / doubleExplosionSize;
    ...
    if (entity instanceof EntityLivingBase)
      finalValue = getBlastReduction((EntityLivingBase)entity, getDamageMultiplied(damage), new Explosion((World)mc.world, null, posX, posY, posZ, 6.0F, false, true)); 
  ...
  public static EnumFacing enumFacing(BlockPos blockPos) {
    ...
    while (i < length) {
      ...
      if ((rayTraceBlocks = mc.world.rayTraceBlocks(vec3d, vec3d2, false, true, false)) != null && rayTraceBlocks.typeOfHit
        .equals(RayTraceResult.Type.BLOCK) && rayTraceBlocks.getBlockPos().equals(blockPos))
        return enumFacing; 
      i++;
    } 
  ...
  public static boolean isEating() {
    return (mc.player != null && (mc.player.getHeldItemMainhand().getItem() instanceof net.minecraft.item.ItemFood || mc.player.getHeldItemOffhand().getItem() instanceof net.minecraft.item.ItemFood) && mc.player.isHandActive());
  }
