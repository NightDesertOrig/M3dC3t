...
public class NoFall extends Module {
  ...
  private final Setting<Integer> distance = register(new Setting("Distance", Integer.valueOf(3), Integer.valueOf(0), Integer.valueOf(50), v -> (this.mode.getValue() == Mode.BUCKET || this.mode.getValue() == Mode.PACKET)));
  ...
  public NoFall() {
    super("NoFallDamage", "Prevents fall damage.", Module.Category.PLAYER, true, false, false);
    setInstance();
  }
  ...
  public void onPacketSend(PacketEvent.Send event) {
    ...
    if (this.mode.getValue() == Mode.PACKET && event.getPacket() instanceof CPacketPlayer) {
      if (mc.player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem().equals(Items.ELYTRA))
        return; 
      if (mc.player.fallDistance >= ((Integer)this.distance.getValue()).intValue()) {
