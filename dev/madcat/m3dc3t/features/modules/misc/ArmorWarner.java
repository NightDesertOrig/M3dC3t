...
public class ArmorWarner extends Module {
  ...
  public ArmorWarner() {
    super("ArmorWarner", "Message friends when their armor is low durable.", Module.Category.MISC, true, false, false);
  }
  
  @SubscribeEvent
  public void onUpdate(UpdateWalkingPlayerEvent event) {
    for (EntityPlayer player : mc.world.playerEntities) {
      if (player.isDead || !M3dC3t.friendManager.isFriend(player.getName()))
        continue; 
      for (ItemStack stack : player.inventory.armorInventory) {
        ...
        if (percent <= ((Integer)this.armorThreshhold.getValue()).intValue() && !this.entityArmorArraylist.containsKey(player)) {
          if (player == mc.player && ((Boolean)this.notifySelf.getValue()).booleanValue()) {
            Command.sendMessage(player.getName() + " watchout your " + getArmorPieceName(stack) + " low durable!");
          } else {
            mc.player.sendChatMessage("/msg " + player.getName() + " " + player.getName() + " watchout your " + getArmorPieceName(stack) + " low durable!");
          } 
